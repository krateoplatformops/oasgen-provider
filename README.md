...
- **External system / External APIs:** The third-party service or API that we want to manage through Kubernetes.
- **Resource reconciliation:** The process of ensuring that the state of resources in the external system matches the desired state defined in Kubernetes which is the source of truth.
...
It must be noted that the **source of truth** for the resource is the resource applied to the Kubernetes cluster, and not the external system.
