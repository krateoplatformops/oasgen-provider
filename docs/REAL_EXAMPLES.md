...
```yaml
    ...
    - '["searchCriteria.creatorId"]'
```
The first line does not need escaping since `completionOptions` is an object in the spec and `triggeredByAutoComplete` is one of its fields.
The second line needs escaping since `searchCriteria.creatorId` is not an object with field `creatorId` in the spec, but rather a single field name including a dot (unusual but possible).
